...
def print_banner():
        print('''\nDescription:
        bucketkicker is a tool to quickly enumerate AWS S3 buckets verify whether 
        or not they exist and to look for loot. It's similar to a subdomain 
        bruteforcer but is made specifically to S3 buckets and also has some 
        extra features that allow you to grep for delicous files as well as 
        download interesting files if you're not afraid to quickly fill up 
        your hard drive.

        by @CraigHays, based on AWSBucketDump by Jordan Potti
        @ok_bye_now'''
        )   
